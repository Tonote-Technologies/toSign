...
<template>
  ...
  <Vue3DraggableResizable v-else :initW="Number(tool.tool_width)" :initH="Number(tool.tool_height)"
    :minW="tool.tool_name == 'Textarea' ? 63 : 95" :minH="24" :x="Number(tool.tool_pos_left)"
    :y="Number(tool.tool_pos_top)" :parent="true" v-model:x="x" v-model:y="y" v-model:h="tool.tool_height"
    v-model:w="tool.tool_width" @drag-end="dragEnd($event, tool)" @resize-end="resizeEnd($event, tool)"
    @activated="print(tool.tool_name)"
    :draggable="comp == 'audit' ? false : profile.id == tool.user_id || owner.isOwner == true" :resizable="comp == 'audit' ? false : profile.id == tool.user_id || owner.isOwner == true
    ...
    :class-name-active="{ 'active-class': tool.tool_name != 'Textarea' }" class-name-dragging="dragging-class"
    class-name-handle="handle-class" class-name-resizing="resizing-class">
    <div v-if="tool.tool_name == 'Textarea'" :style="{ 'height': textHeight + 'px' }">
